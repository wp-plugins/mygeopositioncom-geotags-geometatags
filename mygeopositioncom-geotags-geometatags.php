    ...
    throw new SyntaxError('JSON.parse');};}}());
	  ...
	  */
      ...
      var geopickerUrl = 'http://api.mygeoposition.com/api/geopicker/id-wordpresspipm/?langCode=<?php echo substr(get_locale(), 0, 2); ?>';
          if (document.getElementById('geopicker-ICBM').value) {
            geopickerUrl += '&startAddress=' + document.getElementById('geopicker-ICBM').value;
            ...
            geopickerUrl += '&autolocate=<?php echo (mygpGeotagsGeoMetatags_getOption('autolocate') ? 'true' : 'false'); ?>';
          }
      ...
      mgpGeoWindow.document.write("  window.opener.document.getElementById('geopicker-ICBM').value = geodata.lat + ', ' + geodata.lng;");

      <?php
      if (mygpGeotagsGeoMetatags_getOption('addPosttags')) {
      ?>
      ...
      <?php
      if (mygpGeotagsGeoMetatags_getOption('addCityToPosttags')) {
      ?>
	  ...
	  function clearGeoData() {
	  	  ...
	  	  document.getElementById('geopicker-ICBM').value = '';
...
function mygpGeotagsGeoMetatags_addGeoMetatags() {
    ...
    if (is_single() or is_page()) {
        ...
        if ($wp_query->post && mygpGeotagsGeoMetatags_getOption('addMetatags')) {
 ...
 *
 */
function mygpGeotagsGeoMetatags_getOption($option) {
    
    $value = get_option('mygpGeotagsGeoMetatags_' . $option);
    
    if ($value['text_string'] != '') {
    	
    	return ($value['text_string'] == 'true' ? true : false);
